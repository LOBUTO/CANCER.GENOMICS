...
Function.master.boolnet.cancer<-function(tang.matrix, tcga.mut, paths=c(), layers=1){
  ...
  clusterExport(cl, varlist=c("as.data.table","data.table","tcga.mut", "kegg.path.enzyme","kegg.path", "kegg.edges","path.normal.breast",
  ...
  #Apply NORMAL bfs and boolnet for each individual (parallelize if necessary)
  master.list<-parLapply(cl, tcga.samples, function(x) { 
    print (x)
    
    #Apply NORMAL-BFS (path.normal.breast) - Last used with constant edges
    mut.table<-tcga.mut[SAMPLE==x,]
    sample.bfs<-Function.mut.bfs(kegg.path.enzyme, kegg.path, kegg.edges, path.normal.breast, path = paths , n.layers = layers, mut.table ,cancer = F, met.edges = "constant")
...
path.normal.breast<-readRDS(args[6])

output.file<-args[7]
print ("done loading files")

MAIN.OBJ<-Function.master.boolnet.cancer(tang.matrix, tcga.mut,  c(""), 4)
