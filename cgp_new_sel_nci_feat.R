...
# Prep drug table first (Depending on usage)
if(usage=="nci60"){
  target_nsc   <- as.character(nci_to_cgp[Compound==target_drug,]$NSC)[1] #Fix later
...
# Then merge it all (depending on usage)
if (usage=="nci60"){
  ...
  main_table   <- nci60.gi50[,c("cell_name", "Compound", "NORM.pIC50"), with=F]
}
...
write.table(main_table, paste0(out_table, usage, "_NCI_CGP_SEL_FEAT.", target_drug),
