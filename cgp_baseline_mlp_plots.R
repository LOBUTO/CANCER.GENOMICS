...
Function_create_plot_table <- function(c_range, d_range){
...
all_baseline       <- Function_create_plot_table(c(10, 20, 50, 100, 200, 300, 500, 750, 900),
                                                 c(10, 20, 50, 100, 200))
targetted_baseline <- Function_create_plot_table(c(750), c(2, 4, 6, 8))
...
print(ggplot(unique(all_baseline[,c("Compound", "Cor", "C", "D"),with=F]), aes(x="Correlation", y= Cor)) + geom_boxplot() + geom_jitter(size=0.5) +
...
print(ggplot(unique(all_baseline[,c("Compound", "Cor", "C", "D"),with=F])[,list(MEDIAN=median(Cor)), by=c("C","D")],
             aes(C, MEDIAN, fill=factor(D) )) +
        geom_bar(stat="identity", position="dodge") + scale_fill_brewer(palette = "Set1") + theme_bw() +
        ggtitle("DL CGP Drug vs Cells features in MLP \n MLP applied to entire set \n
                Number of Drug and Cell features pre-filtered prior to model based on variance"))

print(ggplot(unique(targetted_baseline[,c("Compound", "Cor", "C", "D"),with=F]), aes(x="Correlation", y= Cor)) + geom_boxplot() + geom_jitter(size=0.5) +
          facet_grid(C~D) + theme_bw() + xlab("Drug features used for Model") + ylab("Cell features used for Model") +
          ggtitle("DL CGP Drug vs Cells features in MLP \n MLP applied to entire set \n
                  Number of Drug and Cell features pre-filtered prior to model based on variance"))

print(ggplot(unique(targetted_baseline[,c("Compound", "Cor", "C", "D"),with=F])[,list(MEDIAN=median(Cor)), by=c("C","D")],
             aes(C, MEDIAN, fill=factor(D) )) +
        geom_bar(stat="identity", position="dodge") + scale_fill_brewer(palette = "Set1") + theme_bw() +
        ggtitle("DL CGP Drug vs Cells features in MLP \n MLP applied to entire set \n
                Number of Drug and Cell features pre-filtered prior to model based on variance"))
...
##############################################################################################################
# WRITE TABLE
saveRDS(all_baseline, paste0(folder_out, date_out, ".baseline_mlp_C_D.rds"))
