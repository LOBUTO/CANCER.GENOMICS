...
gsea <- fread("/Users/jzamalloa/Documents/Rotation/PIPELINES/GSEA_FILES/c1_sets")
...
main_table <- parLapply(cl, samples, function(i) {
	# write.table(i/length(samples), "/tigress/zamalloa/log.out")

	p_vals <- apply(gene_sets, 1, function(x) {
	...
	count <<-count+1
	write.table(count/length(samples), "/Users/jzamalloa/Documents/Rotation/PIPELINES/GSEA_FILES/log.out")
...
saveRDS(main_table, "/tigress/zamalloa/GSEA_FILES/c1_gsea_cgp_pvals.rds")