...
def weight_variable(shape):
	initial = tf.get_variable(name="w", shape=shape, initializer=tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float32),
							  dtype=tf.float32)
	return initial

def bias_variable(shape):
	initial = tf.get_variable(name="b", shape=shape, initializer=tf.constant_initializer(value=0.1, dtype=tf.float32),
							dtype=tf.float32)
	return initial
...
def dae(cell_train, cell_test, gsea, arch):
	...
	# ENCODING
	genes_i   = list(cell_train.columns)
	g_loc     = {g:[genes_i.index(i) for i in list(gsea[gsea.Gene_set==g].genes)] for g in g_sets}
	...
	with tf.device('/gpu:0'):
		...
		with tf.variable_scope("bias", reuse=tf.AUTO_REUSE):
			main_dict["h0_b"]  = bias_variable([len(g_sets)]) # ADDED

		main_dict["h0_e"]  = lrelu(tf.concat([main_dict["h0_e_%s"%g] for g in g_sets], 1) + main_dict["h0_b"], alpha=0.2)
		...
		for l in xrange(len(layers)-1):
			...
			with tf.variable_scope("encoding", reuse=tf.AUTO_REUSE):
				...
				main_dict["b%s_e"%(l+1)]   = bias_variable([layers[l+1]]) # ADDED

			main_dict["h%s_e"%(l+1)]   = lrelu(tf.matmul(main_dict["h%sd_e"%l], main_dict["W%s_e"%(l+1)] + main_dict["b%s_e"%(l+1)]), alpha=0.2)
	...
	with tf.device('/gpu:0'):
		...
		for k in xrange(len(rev_layers)-1):
			...
			with tf.variable_scope("decoding", reuse=tf.AUTO_REUSE):
				...
				main_dict["b%s_d"%(k+1)]  = bias_variable([rev_layers[k+1]]) # ADDED

			main_dict["h%s_d"%(k+1)]  = lrelu(tf.matmul(main_dict["h%sd_d"%k], main_dict["W%s_d"%(k+1)] + main_dict["b%s_d"%(k+1)]), alpha=0.2)
		...
		# Decode to main
		with tf.variable_scope("last", reuse=tf.AUTO_REUSE):
	...
	print("number of training samples: {}".format(train_feat.shape[0]))
	print("number of testing samples: {}".format(test_feat.shape[0]))
	while (patience < max_patience) and (count < max_training):
		...
		if count_batch == (n_batches-1): # Batch limit count, that is a whole epoch has passed
			...
			# Update validation
			if (test_error + 0.0001) < best_test:
	...
	train_encoded = pd.DataFrame(train_encoded,
								 columns=[str(i)+"_g" for i in xrange(layers[-1])],
	...
	# Return
	return train_encoded, test_encoded, log_table

def gsea_autoencoder(cell_train, cell_test, in_folder, gsea="c2setcover", filter_th=int(10), arch="2"):
	...
	# Manual loading
	cell_train = pd.read_csv("/home/ubuntu/DAE_FILES/gdsc_ctrp_train_gsea_dae_updated.txt", sep="\t", index_col=0)
	cell_test  = pd.read_csv("/home/ubuntu/DAE_FILES/gdsc_ctrp_test_gsea_dae_updated.txt", sep="\t", index_col=0)
	log        = pd.read_csv("/home/ubuntu/DAE_FILES/dae_log_updated.txt", sep="\t")
