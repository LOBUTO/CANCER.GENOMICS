# Script obtained from https://github.com/webermarcolivier/statannot/blob/master/statannot.py
# Adapted for one-sideness of mann-whitney and added wilcoxon (one-sided calculation)
...
def stat_test(box_data1, box_data2, test):
    ...
    if test == 'Mann-Whitney':
        ...
        testShortName = 'M.W.W.'
        formattedOutput = "MWW RankSum one-sided P_val={:.3e} U_stat={:.3e}".format(pval, u_stat)
    elif test == 't-test':
    ...
    elif test == 'wilcoxon': # MODIFIED
        u_stat, pval  = stats.wilcoxon(box_data1, box_data2)
        pval          = pval/2 # MODIFIED - One-sided, greater
        testShortName = 'W.S.R.T.'
        formattedOutput = "Wilcoson SRT paired samples, P_val={:.3e} stat={:.3e}".format(pval, u_stat)
...
def add_stat_annotation(ax,
    ...
    validList = ['t-test', 'Mann-Whitney', 'wilcoxon'] #MODIFIED
