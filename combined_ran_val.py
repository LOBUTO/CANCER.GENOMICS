#HOLD ONE OUT OF MLP FOR DRUG ENTRY
#SMART_BATCH_MET.py
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000, initial_momentum = 0.5,
    ...
    FILE_OUT.write("EPOCH" + "\t" + "TRAIN"+ "\t"+"VALID.NRMSE" + "\t" + "VALID.COR")
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                validation_pears = [validate_cor(i) for i in xrange(n_valid_batches)]
                this_validation_pear = numpy.mean(validation_pears)

                this_train_error = [train_error(i) for i in xrange(n_train_batches)]
                this_train_error = numpy.mean(this_train_error)

                # STORE_RESULTS.write("\n" + str(learning_rate) + "\t" + str(n_epochs) + "\t" + str(train_batch_size) +"\t" +
                #                    str(L1_reg)+"\t"+str(L2_reg)+"\t"+str(N_HIDDEN)+"\t"+str(p)+"\t"+str(dropout)+"\t"+ str(input_p)+"\t"+
                #                     str(epoch) + "\t" + "VALIDATION" + "\t" + str(this_validation_loss))
                # STORE_RESULTS.write("\n" + str(learning_rate) + "\t" + str(n_epochs) + "\t" + str(train_batch_size) +"\t" +
                #                    str(L1_reg)+"\t"+str(L2_reg)+"\t"+str(N_HIDDEN)+"\t"+str(p)+"\t"+str(dropout)+"\t"+ str(input_p)+"\t"+
                #                     str(epoch) + "\t" + "TRAIN" + "\t" + str(this_train_error))

                FILE_OUT.write("\n"+ str(epoch) + "\t" + str(this_train_error) + "\t"+ str(this_validation_loss) +
                                "\t" + str(this_validation_pear))
                print(
                    'epoch %i, minibatch %i/%i, train error %f ,validation error %f %f %%' %
                    (
                        epoch,
                        minibatch_index + 1,
                        n_train_batches,
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    test_cor = [test_COR(i) for i in xrange(n_test_batches)]
                    test_PEAR = numpy.mean(test_cor)
                #
                #     STORE_RESULTS.write("\n" + str(learning_rate) + "\t" + str(n_epochs) + "\t" + str(train_batch_size) +"\t" +
                #                    str(L1_reg)+"\t"+str(L2_reg)+"\t"+str(N_HIDDEN)+"\t"+str(p)+"\t"+str(dropout)+"\t"+ str(input_p)+"\t"+
                #                     str(epoch) + "\t" + "TESTING" + "\t" + str(test_score))
                #
                #
                    print((' epoch %i, minibatch %i/%i, test error of '
                        'best model %f, %f %%') %
                        (epoch, minibatch_index + 1, n_train_batches,
                        test_score, test_PEAR))
                #
                # ONLY save the best model IF VALIDATION IMPROVEMENT!
                # MODEL = [classifier.linearRegressionLayer]
                # for e in xrange(len(n_hidden)):
                #     MODEL = MODEL + [getattr(classifier, "layer_" + str(e))]
                # MODEL = MODEL + [rng]
                # with open( OUT_FOLDER+".MODELS" +"/"+ drug_name + STORE_FILE + ".pkl", 'wb') as f:
...
for niter in [1]:
    for th in ["0.6"]:
        ...
        target=METRIC
        train_drug_x, train_drug_y=shared_drug_dataset_IC50(TRAIN_FILE+"."+INDEX, integers=False, target=target)
        valid_drug_x, valid_drug_y=shared_drug_dataset_IC50(VALID_FILE+".1", integers=False, target=target)
        test_drug_x, test_drug_y=shared_drug_dataset_IC50(TEST_FILE+".1", integers=False, target=target)
        ...
        for drop_out in [0.5]:
            ...
            for l in [2]:
                test_mlp(learning_rate=0.05, L1_reg=0, L2_reg=0.0000000, n_epochs=3000, initial_momentum=0.95, input_p=0.5,
                             datasets=drugval, train_batch_size=50,
                             n_hidden=[300, 300], p=drop_out, dropout=True,
