...
main_table = pd.read_csv(TRAIN_TABLES + "pre_train_ccle_" + target_drug, sep="\t")

# nci_boost = sys.argv[2]
...
with open(TRAIN_TABLES + "TRAIN." + target_drug + ".ccle.pIC50.pkl", "wb") as tr:
    cPickle.dump(train_scaled, tr)

with open(TRAIN_TABLES + "VALID." + target_drug + ".ccle.pIC50.pkl", "wb") as vd:
    cPickle.dump(valid_scaled, vd)

with open(TRAIN_TABLES + "TEST." + target_drug + ".ccle.pIC50.pkl", "wb") as tc:
    cPickle.dump(test_scaled, tc)

std_dict = {"mean" : std_scale.mean_, "std" : std_scale.std_}
with open(TRAIN_TABLES + "STD_SCALER." + target_drug + ".ccle.pIC50.pkl", "wb") as sc:
    cPickle.dump(std_dict, sc)

print("Done post-treatment")
