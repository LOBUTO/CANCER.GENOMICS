...
Function.CLUSTERS.MI.WICOXON<-function(filtered.normal.mi, cancer.mi, figures.folder, gene.pos, samples) {
  ...
  main.wilcox<-main.table[,list(P.VAL=wilcox.test(MI, CANCER.MI, paired=T)$p.value),
                         by="CLUSTER"]
  ...
  for (cluster in main.list) {
    ...
    if (nrow(cluster)<100){
      ...
      filename=paste(gene.pos, cluster.name, "jpeg",sep=".")
...
###TESTING###
test.process.file<-Function.Process.Cluster("PIPELINES/METABOLIC.DRIVERS/NETWORKS/SPICI.RESULTS/BRCA.EXP.NORMAL.TH.0.95.cluster")
...
TTN.NO.PIK3CA<-setdiff(TTN, PIK3CA)
...
PIK3CA.NO.TTN.cluster.mi.wilcoxon<-Function.CLUSTERS.MI.WICOXON(test.simplify, PIK3CA.NO.TTN.cohort.cluster.mi, 
                                                                "FIGURES/LINKAGE/MI/PIK3CA.NO.TTN.0.95", "PIK3CA.NO.TTN.0.95",PIK3CA.NO.TTN)

TTN.NO.PIK3CA.cohort.cluster.mi<-Function.CANCER.COHORT.CLUSTER.MI(test.process.file, TTN.NO.PIK3CA, brca.exp.nb$combined.matrices)

TTN.NO.PIK3CA.cluster.mi.wilcoxon<-Function.CLUSTERS.MI.WICOXON(test.simplify, TTN.NO.PIK3CA.cohort.cluster.mi, 
                                                                "FIGURES/LINKAGE/MI/TTN.NO.PIK3CA.0.95", "TTN.NO.PIK3CA.0.95",TTN.NO.PIK3CA)
