...
Function.Main<-function(maf, exp.matrix){
  ...
  #Filter for those that genes that are found to be mutated in at least 10 individuals
  maf[,N.SAMPLES:=length(unique(SAMPLE)), by="Hugo_Symbol"]
  maf<-maf[N.SAMPLES>=10,]
  maf$N.SAMPLES<-NULL
