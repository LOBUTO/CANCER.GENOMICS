...
Function.BRCA.Clinical<-function(clinical_patient.file, clinical_cqcf.file){
    ...
    clinical<-clinical[,c(1,7,9,14,15,16,34,35,41,44,50,56), with=F]
    setnames(clinical, c("PATIENT", "GENDER", "RACE", "VITAL.STATUS", "DAYS.TO.LAST.CONTACT", "DAYS.TO.DEATH", "EXAMINED.LYMPH.NODES", "POSITIVE.LYMPH.NODES","TUMOR.STAGE", "ER.STATUS", "PR.STATUS","HER2.STATUS"))
    ...
    main.clinical$TUMOR.STAGE.SET<-ifelse(main.clinical$TUMOR.STAGE %in% c("Stage I", "Stage IA", "Stage IB", "Stage IC"), "Stage I", 
        ...
        ifelse(main.clinical$TUMOR.STAGE %in% c("Stage IV", "Stage IVA", "Stage IVB"), "Stage IV", as.character(main.clinical$TUMOR.STAGE) ))))
