...
do
  ...
  do
    ...
    do
      ...
      echo $c $d $samples
      file_name="${samples}_scaled_C_${c}_MC_${d}_mf_${multiplicative_fusion}_dn_${drug_n}_cn_${cell_n}_fn_${fusion_n}"

      # Prep training sets
      if [ "$multiplicative_fusion" == "T" ]
      then
        Rscript GIT/cgp_new_prep_mf.R $c $d $file_name $met_type $class_mlp $samples

        script_name="GIT/cgp_multi_mlp.py"

        file_name="${file_name} ${drug_n} ${cell_n} ${fusion_n} ${class_mlp} ${d} ${c}"
        ...
        Rscript GIT/cgp_new_prep.R $c $d $file_name $met_type $class_mlp $samples
